...
while i < 4:
    j = 0
    p = 0
    # вот этот алгоритм настолько непонятен в частности из-за обозначений, что я не понял, что он делает
    if M[i][0] - xc >= 0:
        p += 1
...
# так сделать можно, но лучше использовать стандартную матрицу перехода
...
# оценка 3/10 т.к. 
# 1) алгоритм в середне я сходу не понял, я кто-то может не захотеть разбираться дальше
# 2) "Напишите функцию..."
