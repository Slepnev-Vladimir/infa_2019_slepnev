from graph import polygon, circle, rectangle, polyline

from graph import penColor, brushColor

from graph import windowSize, penSize

from graph import changeCoords
...
def cat(x0, y0, szx, szy, c):
    ...
    changeCoords(circle_base, [(0.8 * szx + x0, 0.6 * szy + y0), (25.6 * szx + x0, 22.6 * szy + y0)])

    # yshi
    polygon([(x0, y0), (6.4 * szx + x0, 3 * szy + y0), (1.8 * szx + x0, 8 * szy + y0)])
    polygon([(23.8 * szx + x0, -1 * szy + y0), (17.8 * szx + x0, 2.6 * szy + y0), (22.6 * szx + x0, 7.4 * szy + y0)])

    penSize(0.5)
    brushColor(222, 170, 135)
    polygon([(szx + x0, szy + y0), (5.4 * szx + x0, 3 * szy + y0), (2.2 * szx + x0, 6.6 * szy + y0)])
    polygon([(23 * szx + x0, y0), (18.6 * szx + x0, 2.8 * szy + y0), (22.4 * szx + x0, 6 * szy + y0)])
    ...
    changeCoords(circle_base, [(8.4 * szx + x0, 9.5 * szy + y0), (9.2 * szx + x0, 15.5 * szy + y0)])
    circle_base = circle(185, 646, 30)
    changeCoords(circle_base, [(19 * szx + x0, 9.5 * szy + y0), (19.8 * szx + x0, 15.5 * szy + y0)])
    ...
    penColor("black")
    brushColor(222, 170, 135)
    polygon([(12 * szx + x0, 16.6 * szy + y0), (14.2 * szx + x0, 16.6 * szy + y0), (13.2 * szx + x0, 17.8 * szy + y0)])
...
def window(x1, y1, x2, y2):
    ...
    penColor(0, 231, 255)
    brushColor(141, 207, 255)
    rectangle(0.95 * x1 + 0.05 * x2, 0.95 * y1 + 0.05 * y2, 0.55 * x1 + 0.45 * x2, 0.75 * y1 + 0.25 * y2)
    rectangle(0.45 * x1 + 0.55 * x2, 0.95 * y1 + 0.05 * y2, 0.05 * x1 + 0.95 * x2, 0.75 * y1 + 0.25 * y2)
    rectangle(0.95 * x1 + 0.05 * x2, 0.67 * y1 + 0.33 * y2, 0.55 * x1 + 0.45 * x2, 0.05 * y1 + 0.95 * y2)
    rectangle(0.45 * x1 + 0.55 * x2, 0.67 * y1 + 0.33 * y2, 0.05 * x1 + 0.95 * x2, 0.05 * y1 + 0.95 * y2)
...
window(300, 15, 450, 185)
window(100, 15, 250, 185)
window(-90, 15, 60, 185)
ball(142, 240, 2, 2)
ball(374, 344, -2, 2)
ball(294, 383, -3, 3)
