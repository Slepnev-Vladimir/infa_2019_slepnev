...
class Ball(Agent):
    def __init__(
        ...
        self.live = 300 if live is None else live
...
class Gun(Agent):
    def __init__(self, canvas):
        ...
        self.gun_velocity = 1
        self.gun_power_gain = 0.1
        self.min_gun_power = 1
        self.max_gun_power = 12
        self.zero_power_length = 20
        self.power_length_coeff = 10
    ...
    def get_gunpoint(self):
        length = self.power_length_coeff * self.f2_power + self.zero_power_length
...
class Target(Agent):
    def __init__(
            self, canvas, x=None, y=None, r=None, color=None, job_init=None, vx=None, vy=None):
        super().__init__()
        self.job = job_init

        x = self.x = rnd(20, 720) if x is None else x
        y = self.y = rnd(0, 450) if y is None else y
        r = self.r = rnd(2, 30) if r is None else r
        vx = self.vx = rnd(-3, 3) if vx is None else vx
        vy = self.vy = rnd(-3, 3) if vy is None else vy
...
class BattleField(tk.Canvas):
    ...
    def show_victory_text(self):
        if self.last_hit_bullet_number > 1:
            self.itemconfig(
                        self.victory_text_id,
                        text='you have destroyed all targets for '+str(self.last_hit_bullet_number)+' shots')
        else:
            self.itemconfig(
                        self.victory_text_id,
                        text='you have destroyed all targets for '+str(self.last_hit_bullet_number)+' shot')
